...
Project Flutter yang dibuat merupakan aplikasi sederhana yang menampilkan beberapa contoh penggunaan widget antarmuka (UI) di Flutter. Pada bagian `MyApp`, aplikasi dibungkus menggunakan `MaterialApp` untuk memberikan tampilan bergaya Material Design. Selanjutnya, halaman utama aplikasi ditampilkan melalui `HomePage`. Di dalam `HomePage`, seluruh komponen ditampilkan secara vertikal menggunakan `SingleChildScrollView` dan `Column`, sehingga pengguna dapat melakukan scroll untuk melihat setiap contoh widget. Setiap bagian memiliki judul dan contoh implementasi widget yang berbeda.
...
Widget `Container` digunakan untuk membuat sebuah kotak yang dapat diberi warna, ukuran, padding, border, dan bayangan. Pada kode ini, Container dibuat dengan latar biru muda, sudut membulat, serta efek shadow agar tampil lebih menarik. Di dalamnya terdapat ikon dan teks yang menjelaskan fungsi `Container`. Bagian ini menunjukkan bahwa `Container` sangat berguna untuk membungkus dan mempercantik tampilan suatu komponen agar terlihat lebih terstruktur dan rapi.
...
`GridView` digunakan untuk menampilkan data dalam bentuk susunan grid atau kotak-kotak. Pada program ini digunakan `GridView.count` dengan tiga kolom dan total enam item. Setiap item memiliki warna, ikon, dan teks yang berbeda sehingga tampilan menjadi lebih variatif. Penggunaan `GridView` cocok untuk menampilkan data visual seperti menu aplikasi, galeri gambar, atau daftar kategori karena tampilannya lebih ringkas dan teratur.
...
`ListView` digunakan untuk menampilkan daftar item secara vertikal. Pada bagian ini, list berisi tiga item sederhana yaitu Item A, Item B, dan Item C. Setiap item dibuat menggunakan `ListTile` yang memiliki ikon, judul, subtitle, dan ikon panah di sebelah kanan. Penggunaan `ListView` biasa cocok ketika jumlah data sedikit dan item dapat ditulis langsung di dalam `children`.
...
`ListView.builder` digunakan untuk membuat daftar secara dinamis berdasarkan data yang ada pada array `itemList`. Widget ini lebih efisien karena item hanya dibuat saat diperlukan ketika pengguna melakukan `scroll`. Pada program ini, setiap item menampilkan ikon, judul, subtitle, dan nomor urut. Data diambil secara otomatis dari array sehingga penambahan atau pengurangan data dapat dilakukan dengan lebih mudah tanpa perlu menulis item satu per satu.
...
`ListView.separated` hampir sama dengan `ListView.builder`, tetapi memiliki tambahan pemisah antar item. Pada kode ini digunakan `Divider` sebagai garis pembatas agar setiap item terlihat lebih jelas dan rapi. Daftar tetap dibuat dari data array yang sama, namun tampilannya menjadi lebih terorganisir karena adanya separator. Widget ini cocok digunakan pada daftar data yang panjang agar pengguna lebih mudah membedakan antar item.
...
`Stack` digunakan untuk menumpuk beberapa `widget` dalam satu area. Pada program ini terdapat beberapa kotak dengan ukuran dan warna berbeda yang disusun saling bertumpuk. Widget `Positioned` digunakan untuk mengatur posisi tertentu seperti di pojok atas atau bawah. Contoh ini menunjukkan bahwa `Stack` sangat berguna untuk membuat tampilan berlapis, seperti badge notifikasi, banner, atau desain antarmuka yang membutuhkan elemen saling menimpa.
